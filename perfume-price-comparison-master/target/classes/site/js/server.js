...
const handlegetFragrance = (request, response) => {
  connectionPool.query("SELECT * FROM fragrances", (err, result) => {
    if (err) {
...
const handlegetbrand = (req, res) => {
  ...
  let sqlPerfumeInBrand = "SELECT * FROM fragrances WHERE brand='" + brand + "'";
...
const handlegetAllbrands=(req,res)=>{
  const query = 'SELECT DISTINCT Brand FROM fragrances';
...
const handleGetPerfumeByName=(req, res)=>{
  ...
  const query = "SELECT * FROM fragrances WHERE name = '" + perfName + "'";
